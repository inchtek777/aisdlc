...
description: Use when 需要因需求重大问题而废弃/撤销当前 Spec Pack，并且必须删除对应 `.harness/specs/{branch}` 目录与本地/远程分支，同时需要在执行删除前输出删除清单并要求用户二次确认。
---
...
- **使用时机**
  ...
  - 你需要删除：`.harness/specs/{CURRENT_BRANCH}/` 以及该 `{CURRENT_BRANCH}` 分支（本地 + 远程）。
- **不要用在**
